...
public class AstahStateMachineDiagramToHash
{
    ...
    public HashMap<String, HashMap> getHash() throws Exception{
        ...
        if (mDiagram instanceof IStateMachineDiagram ) {
            System.out.println( "Getting Information from " + mDiagram.getFullName("::") );
    ...
    private HashMap getStateMachineDiagramInfo(IStateMachineDiagram iStateMachineDiagram) throws Exception {
        ...
        for (IVertex iVertex : iVertexes ) {
            if( iVertex instanceof IPseudostate ) {
                ...
                if( iPseudostate.isInitialPseudostate() ) {
                    initialState = (IState)(iVertex.getOutgoings()[0].getTarget());
                } else {
    ...
    private HashMap getStateInfo( IState iState ) throws Exception {
        ...
        for( ITransition iTransition : iState.getInternalTransitions() ) {
            transitions.put( iTransition.getId(), getInternalTransitionInfo( iTransition  ) );
        }
        for( ITransition iTransition : iState.getOutgoings() ) {
            ...
            if( !transitions.containsKey( iTransition.getId() ) ) {
                transitions.put( iTransition.getId(), getTransitionInfo( iTransition ) );
            }
        }
    ...
    private HashMap getInternalTransitionInfo( ITransition iTransition  ) throws Exception {
        // 内部遷移を追加
    ...
    private HashMap getTransitionInfo( ITransition iTransition  ) throws Exception {
        HashMap<String, String> transition = new HashMap<String, String>();
        ...
        if( target instanceof IState ) {
            ...
            if( iPseudostate.isInitialPseudostate() ) {
                ...
                throw new Exception("ShallowHistoryPseudostate does not supported.");
            } else if( iPseudostate.isDeepHistoryPseudostate() ) {
                // 深い履歴状態を用いてpopを表現する
                ...
                if( es.length > 1 ) {
                    if( es[1].equals( "Push" ) ) {
                        transition.put( "Target", "push(" + iPseudostate.getContainer() + "::" +
                                        target.getName() + ")" );
                        event = es[0];
                    } else {
                    ...
                    transition.put( "Target", "jump(" + iPseudostate.getContainer() + "::" +
                                    target.getName() + ")" );
                }
            }
